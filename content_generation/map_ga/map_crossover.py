...
class MapCrossover:
    ...
    def __create_offspring(self, parent1: List[SolutionArea], parent2: List[SolutionArea]) -> dict:
        ...
        if len(parent1) > len(parent2):
            return self.__k_point_crossover(k=1, shortest=parent2, longest=parent1)
        else:
            return self.__k_point_crossover(k=1, shortest=parent1, longest=parent2)

    def __k_point_crossover(self, k: int, shortest: List[SolutionArea], longest: List[SolutionArea]) -> dict:
        points = set()
    ...
    def __uniform_crossover(self, shortest: List[SolutionArea], longest: List[SolutionArea]) -> dict:
        child1 = []
        child2 = []
        for i in range(0, len(longest)):
