...
class TypeInitialPopulation:
    ...
    def create(self, districts: List[List[tuple]]) -> SolutionType:
        list_of_area = []
        for district in districts:
            area_type = self.get_random_type()
            current_area = AreaType(area_type=area_type, coordinates=district)
            list_of_area.append(current_area)
        return SolutionType(fitness=0, population=list_of_area)
